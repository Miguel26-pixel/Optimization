...
def fi(xi, ap, am, z_list, i, j):
    return 2 * (ap**2) * sum(xi - z_i_j for z_i_j in z_list[:i]) + 2 * (am**2) * sum(xi - z_i_j for z_i_j in z_list[j:])
...
for node in G.nodes:
    ...
    result = root_scalar(fi, args=(int(ap), int(am), z_list, i, j), bracket=[-255, 255])
