...
def fi(xi, ap, am, z_list, i, j):
    return ap * sum(xi - z_i_j for z_i_j in z_list[:j-1]) + am * sum(xi - z_i_j for z_i_j in z_list[j:])
...
def get_j_value(G,node,rop):
    return (len(list(G.neighbors(node)))/(1-rop))+1
...
for node in G.nodes:
    ...
    value = get_j_value(G,node,rop)

    if isinstance(value, int) and value >= 1:
        random_neg = random.randint(-3000, -1)
        random_pos = random.randint(1, 3000)

        result = root_scalar(fi, args=(random_pos, random_neg, z_list_sorted, i, j), bracket=[-255, 255])
        ...
        int_value = int(value)
        if int_value > len(list(G.neighbors(node))):
            int_value = len(list(G.neighbors(node)))-1
        new_image[i][j] = z_list_sorted[int_value]
